...
from .models import (
    ...
    ProjectMembership, Message, TECH_CHOICES, TechStackAnalysis, Hackathon, HackathonRegistration
)
...
class HackathonListView(LoginRequiredMixin, ListView):
    ...
    model = Hackathon
    template_name = 'dashboard/hackathons/list.html'
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['status_choices'] = Hackathon._meta.get_field('status').choices
        return context

class HackathonDetailView(LoginRequiredMixin, DetailView):
    """View for viewing a specific hackathon"""
    model = Hackathon
    template_name = 'dashboard/hackathons/detail.html'
    context_object_name = 'hackathon'
    pk_url_kwarg = 'hackathon_id'
    ...
    def get_context_data(self, **kwargs):
        ...
        context['is_registered'] = HackathonRegistration.objects.filter(
            hackathon=hackathon,
            team__in=user_groups
        ).exists()
        
        return context

class RegisterForHackathonView(LoginRequiredMixin, View):
    """View for registering a team for a hackathon"""
    
    def post(self, request, hackathon_id, group_id):
        hackathon = get_object_or_404(Hackathon, id=hackathon_id)
        group = get_object_or_404(Group, id=group_id)
        
        # Check if user is a member of the group
        if not group.members.filter(id=request.user.id).exists():
            messages.error(request, "You must be a member of the team to register for a hackathon.")
            return redirect('projects:hackathon_detail', hackathon_id=hackathon_id)
            
        # Check if registration deadline has passed
        if timezone.now() > hackathon.registration_deadline:
            messages.error(request, "Registration deadline has passed for this hackathon.")
            return redirect('projects:hackathon_detail', hackathon_id=hackathon_id)
            
        # Check if team is already registered
        if HackathonRegistration.objects.filter(hackathon=hackathon, team=group).exists():
            messages.error(request, "Your team is already registered for this hackathon.")
            return redirect('projects:hackathon_detail', hackathon_id=hackathon_id)
        ...
        messages.success(request, "Successfully registered your team for the hackathon!")
        return redirect('projects:hackathon_detail', hackathon_id=hackathon_id)

class MyHackathonsView(LoginRequiredMixin, ListView):
    """View for listing hackathons a user's teams are registered for"""
    model = HackathonRegistration
    template_name = 'dashboard/hackathons/my_hackathons.html'
    context_object_name = 'registrations'
    paginate_by = 10

    def get_queryset(self):
        user_groups = Group.objects.filter(members=self.request.user)
        return HackathonRegistration.objects.filter(team__in=user_groups).select_related(
            'hackathon', 'team'
        ).order_by('-created_at')
